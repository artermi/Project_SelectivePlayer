...
class BFAPGG: public FAPGG
{
public:
	 BFAPGG(const double rate,const double para):FAPGG(rate,para){
	 	build_specials(para);
	 }

	 void build_specials(const double para);
};
void BFAPGG::build_specials(const double para){
	for(int i = 0; i < LL; i++)
		probs[i] = (double) ((double)rand()/(double)RAND_MAX < para);
	memset(dir_name,'\0',sizeof(dir_name));
	strcpy(dir_name,"bimod");
	P = para;
}
