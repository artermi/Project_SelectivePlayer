...
def do_all_mode():
    rlist = [#2.4,2.41,2.42,2.43,2.44,2.45,2.46,2.47,2.48,2.49,2.5,
            #2.6,2.7,
            #2.8,#2.9,
            #3,
            #3.1,3.15#,
            #3.17,3.2#,
            #3.25,
            #3.3,3.35,3.4,3.5,3.6,
            3.74,3.745,3.75,3.76#,3.78,3.80,3.82,3.84,3.86,3.88,3.90,
            #3.92,3.94,3.96,3.98,
            #4.00,4.05,4.10,4.15,4.20,4.30,4.40,4.50,
            #4.60,4.70,4.80,4.90,5.00,5.10,
            #5.20,5.30,5.40,5.44,5.49,5.5
            ]
    alps = [0.5]
    paths = []
    for alp in alps:
        path = 'BI' + str(int(alp * 100)).zfill(3)
    ...
    for path in paths:
        ...
        for r in rlist:
            ...
            game = FAPGG_5G(r,0.5,200,alp) #r,K,L alp
            per_c = 0.5
...
def do_alpha_mode():
    ...
    for path in paths:
        ...
        for alp in alps:
            filename = p + '/' + 'exp_' + str(int(alp * 10)).zfill(3) + '_'
            filename += 'r_' + str(int( r * 1000)) + '.dat'
            f = open(filename,"w")
            print('Now doing:' + filename)
