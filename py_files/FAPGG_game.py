...
#                temp_matrix.append(APlayer(choices([True,False],[0.1,0.9])[0],alp))
    ...
    def one_play(self,i,j,rnd):
        ...
        goods+= mat[(i+1) % L,j].allocate(4,rnd)
        goods+= mat[(i-1) % L,j].allocate(3,rnd)
        goods+= mat[i,(j+1) % L].allocate(1,rnd)
        goods+= mat[i,(j-1) % L].allocate(2,rnd)
    ...
    def two_players_play(self,rnd):
        ...
        profit_x += self.one_play((xi+1) % L,xj,rnd)
        profit_x += self.one_play((xi-1) % L,xj,rnd)
        profit_x += self.one_play(xi,(xj+1) % L,rnd)
        profit_x += self.one_play(xi,(xj-1) % L,rnd)

        profit_x -= 5 * mat[xi,xj].isCoop
        ...
        profit_y += pye + pyw + pyn + pys

        profit_y -= 5 * mat[yi,yj].isCoop
...
def do_all_mode():
    rlist = [#2.4,2.41,2.42,2.43,2.44,2.45,2.46,2.47,2.48,2.49,2.5,2.6,2.7,2.8,2.9,
            ...
            #3.3,3.35,3.4,3.5,
            #3.6#,
            #3.74,3.747,3.748,3.75,3.76,3.78,
            #3.80,3.82,
            #3.84,3.86,
            3.88,
            #3.90,
